...
export function SubjectsPage() {
  ...
  if (isLoading) return <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{Array.from({ length: 6 }).map((_, i) => <Skeleton key={i} className="h-48 rounded-lg" />)}</div>
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        action={isAdmin ? (
          <Button asChild size="sm"><Link href="/subjects/new"><Plus className="mr-2 h-4 w-4" />Tambah</Link></Button>
        ) : undefined}
      />
      {subjects.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {subjects.map((s) => (
            <SubjectCard key={s.id} subject={s} isAdmin={isAdmin}
...
function SubjectCard({ subject: s, isAdmin, onDelete }: { subject: Subject; isAdmin: boolean; onDelete: () => Promise<void> }) {
    ...
    <Card className="overflow-hidden group">
      <Link href={`/subjects/${s.id}`}>
        <div className="aspect-video bg-muted">
          {thumb
            ? <img src={thumb} alt={s.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
            : <div className="w-full h-full flex items-center justify-center"><BookText className="h-10 w-10 text-muted-foreground/30" /></div>}
        </div>
      </Link>
      <CardContent className="p-4">
        <div className="flex items-start justify-between gap-2">
          <Link href={`/subjects/${s.id}`}>
            <h3 className="font-semibold truncate hover:underline">{s.name}</h3>
          </Link>
          <Badge variant={s.type === 'wajib' ? 'default' : 'secondary'} className="shrink-0">{s.type}</Badge>
        </div>
        <p className="text-sm text-muted-foreground line-clamp-2 mt-1">{s.description}</p>
        {isAdmin && (
          <div className="flex gap-2 mt-3 pt-3 border-t">
            <Button asChild variant="outline" size="sm" className="flex-1">
              <Link href={`/subjects/${s.id}/edit`}><Pencil className="mr-1.5 h-3.5 w-3.5" />Edit</Link>
            </Button>
            <ConfirmDialog
              trigger={<Button variant="outline" size="sm" className="text-destructive border-destructive/30">Hapus</Button>}
              title="Hapus Mata Pelajaran" description={`Yakin menghapus "${s.name}"?`}
...
export function SubjectDetailPage({ id }: { id: string }) {
  ...
  if (!subject) return <p className="text-muted-foreground">Mata pelajaran tidak ditemukan.</p>
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="pt-4 space-y-4">
          {thumb && <img src={thumb} alt={subject.name} className="w-full max-h-64 object-cover rounded-lg" />}
...
function SubjectForm({ defaultValues, onSubmit, isPending, submitLabel, onCancel }: {
  ...
  const { register, handleSubmit, setValue, watch, formState: { errors } } = useForm<SubjectFormValues>({
    resolver: zodResolver(subjectSchema), defaultValues,
  })
  ...
  return (
    <div className="space-y-6 max-w-xl">
      ...
      <Card><CardContent className="pt-6">
        <SubjectForm onSubmit={async (v) => { await mutation.mutateAsync(v); router.push('/subjects') }}
...
export function EditSubjectPage({ id }: { id: string }) {
  ...
  return (
    <div className="space-y-6 max-w-xl">
      ...
      <Card><CardContent className="pt-6">
        <SubjectForm
          defaultValues={subject ? { name: subject.name, type: subject.type, description: subject.description, departmentId: subject.departmentId ?? undefined } : undefined}
          onSubmit={async (v) => { await mutation.mutateAsync(v); router.push(`/subjects/${id}`) }}
